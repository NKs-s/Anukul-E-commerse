...
function Navbar() {
  ...
  return (
    <div className="bg-nav-bg-color sticky top-0 z-50">
            ...
            >
              <Dialog.Panel className="relative flex w-full max-w-xs flex-col overflow-y-auto bg-white pb-12 shadow-xl" style={{ backgroundColor: mode === 'dark' ? 'rgb(40, 44, 52)' : '', color: mode === 'dark' ? 'white' : '', }}>
                ...
                <div className="space-y-6 border-t border-gray-200 px-4 py-6">
                  ...
                  {user?.user?.email === "admin.nks@gmail.com" ? <div className="flow-root">
      ...
      <header className="relative bg-white">
        ...
        <nav aria-label="Top" className="bg-nav-bg-color px-4 sm:px-6 lg:px-8 shadow-xl " style={{ color: 'white'}}>
          <div className="">
            <div className="flex h-16 items-center">
              <button
                type="button"
                className="rounded-md bg-nav-bg-color p-2 text-white-800 lg:hidden"
              ...
              <div className="ml-auto flex items-center">
                <div className="hidden lg:flex lg:flex-1 lg:items-center lg:justify-end lg:space-x-6">
                  ...
                  {user ?  <Link to={'/order'} className="text-sm font-medium text-white " style={{ color: mode === 'dark' ? 'white' : '', }}>
                    ...
                    {user ?  <></> :   <Link to={'/login'}  className="text-sm font-medium text-white " style={{ color: mode === 'dark' ? 'white' : '', }}>
                      Login
                    </Link>}

                  {user?.user?.email === 'admin.nks@gmail.com' ? 
                ...
                {user ?  <div className="ml-4 flow-root lg:ml-6">
                  <Link to={'/cart'} className="group -m-2 flex items-center p-2" style={{ color: mode === '' ? 'white' : '', }}>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                      <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
                    </svg>

                    <span className="ml-2 text-sm font-medium text-white group-" style={{ color: mode === '' ? 'white' : '', }}>{cartItems.length}</span>
