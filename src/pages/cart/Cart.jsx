...
function Cart() {
    ...
    useEffect(() => {
        ...
        cartItems.forEach((cartItem) => {
            temp += parseFloat(cartItem.price.replace(/,/g, ''));
        });
        setTotalAmount(temp);
    }, [cartItems]);
    ...
    const buyNow = async () => {
        ...
        try {
            ...
            const docRef = await addDoc(orderRef, {
                ...
                status: "pending",
                // Include the order ID in the document
                orderId: null, // Placeholder for the order ID
            });

            // Access the ID of the newly added order document
    ...
    return (
        <Layout>
            <div className="h-screen bg-gray-100 pt-5 overflow-y-auto" style={{ backgroundColor: mode === 'dark' ? '#282c34' : '', color: mode === 'dark' ? 'white' : '' }}>
                ...
                <div className="mx-auto max-w-5xl justify-center px-6 md:flex md:space-x-6 xl:px-0 ">
                    ...
                    <div className="mt-6 h-full rounded-lg border bg-white p-6 shadow-md md:mt-0 md:w-1/3" style={{ backgroundColor: mode === 'dark' ? 'rgb(32 33 34)' : '', color: mode === 'dark' ? 'white' : '', }}>
                        ...
                        <div className="flex justify-between">
                            ...
                            <p className="text-gray-700" style={{ color: mode === 'dark' ? 'white' : '' }}>Not Determined</p>
                        </div>
                        <hr className="my-4" />
                        ...
                        {(cartItems.length > 0 && grandTotal >= 1) && (
                            <Modal
                                name={name}
                                address={address}
